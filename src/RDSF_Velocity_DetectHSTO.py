...
def get_heel_toe_arr(vicon, subject, start_frame=1, end_frame=None):
...
def compute_walk_dir(sacrum_arr, frame_rate):
    ...
    if speed > 1e-8:
        ...
        R = np.eye(2) # Sacrum's average velocity is already the same direction as X-axis
...
def compute_projected_velocity(L_heel_arr, L_toe_arr, R_heel_arr, R_toe_arr, R, sacrum_arr, frame_rate):
...
def main():
    ...
    # 2. Get Marker Trajectories
    L_heel_arr, L_toe_arr, R_heel_arr, R_toe_arr, sacrum_arr = get_heel_toe_arr(vicon, subject, start_frame, end_frame)

    # 3. Compute Walking Direction
    R, walk_dir = compute_walk_dir(sacrum_arr, frame_rate)
    ...
    L_heel_vX, L_toe_vX, R_heel_vX, R_toe_vX = compute_projected_velocity(L_heel_arr, L_toe_arr, R_heel_arr, R_toe_arr, R, sacrum_arr, frame_rate)
    ...
    L_heel_strikes, L_toe_offs, R_heel_strikes, R_toe_offs = detect_events(L_heel_vX, L_toe_vX, R_heel_vX, R_toe_vX, start_frame)

    # 7. Visualize Events to Matplotlib
    visualize_matplotlib(L_heel_vX, L_toe_vX, R_heel_vX, R_toe_vX, L_heel_strikes, L_toe_offs, R_heel_strikes, R_toe_offs, start_frame, end_frame)
